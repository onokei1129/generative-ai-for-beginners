...
6. [バックアップ](#6-バックアップ)
7. [復元と復元訓練](#7-復元と復元訓練)
8. [監査ログのアーカイブ](#8-監査ログのアーカイブ)
9. [スキーマ変更（マイグレーション）](#9-スキーマ変更マイグレーション)
10. [監視項目](#10-監視項目)
11. [障害時の対応](#11-障害時の対応)
...
検索用の索引（`pg_trgm` の GIN 索引）はマイグレーションが自動で作ります。
`pg_trgm` は PostgreSQL 13 以降 trusted 拡張のため、通常はアプリの利用者が
そのまま作成できます。マネージドサービス等で拡張の作成が制限されている場合、
マイグレーションは索引を見送って成功しますが、名刺が数万件を超えると検索が
遅くなります。その場合は権限のある利用者で先に次を実行してください。

```bash
sudo -u postgres psql -d bcards -c "CREATE EXTENSION IF NOT EXISTS pg_trgm;"
```
...
| `BCARDS_STORAGE_USAGE_CACHE_SECONDS` | `300` | 使用量の集計をキャッシュする秒数。**S3 運用では小さくしないこと**（バケット全体のリストが毎回走ります） |
...
第1回の実施記録は [restore-drill-2026-07.md](./restore-drill-2026-07.md) にあります。
この訓練で「取得元と復元先でディレクトリ名が違うと画像が別の場所へ展開される」不具合を
検出し、修正しました。自動テストでは見つからず、手順を実際に通したことで見つかった不具合です。

その後、本番相当のデータ量（名刺 10,000 件・画像 30,000 件・1.1GB＝約3.3年ぶん）でも
測定しました（[capacity-report-2026-07.md](./capacity-report-2026-07.md)）。
...
実測が約1分のため、RTO の初期案「4営業時間」は保守的すぎます。
**2営業時間**への短縮を提案しています（決裁は論点O）。
なお、障害の検知・原因判断・関係者への連絡にかかる人的な時間は別途見込んでください。

---

## 8. 監査ログのアーカイブ

要件§9 の監査ログは追記専用で、通常の操作では更新・削除しません。
放置すると件数が増え続けるため、期間で区切って退避します（open-issues 論点H）。

| 対象 | 扱い |
| --- | --- |
| 監査ログ（`audit_log`） | 1年経過でアーカイブ、3年で破棄（いずれも設定で変更可） |
| 変更履歴（`change_history`） | **無期限**。名刺データに保存期限がないため、履歴だけ消さない |
| CSV出力ログ・ログイン履歴 | 監査ログに準じる |

期間は管理画面のシステム設定で変更します。`0` を指定すると無効になります。

| 設定キー | 既定 | 意味 |
| --- | --- | --- |
| `audit_log_archive_after_days` | 365 | この日数を過ぎたログをアーカイブへ移す。`0` で移さない |
| `audit_log_retention_days` | 1095 | アーカイブの保持期間。`0` で無期限 |
...
# 対象件数の確認だけ
.venv/bin/python ops/archive_logs.py --dry-run

# 実行（--all を付けると対象がなくなるまで繰り返す）
.venv/bin/python ops/archive_logs.py --all
```

日次バッチの例:

```cron
# /etc/cron.d/bcards-archive
30 3 * * * bcards . /etc/bcards/env; /opt/bcards/app/.venv/bin/python /opt/bcards/app/ops/archive_logs.py >> /var/log/bcards-archive.log 2>&1
```

管理画面 `/admin/audit/archives` からも実行でき、アーカイブの一覧確認と
個別ダウンロード（JSON Lines の gzip）ができます。

### 安全側の設計

- **書き出しに成功してからDBの行を削除**します。逆順だと書き出し失敗時にログが失われます
- アーカイブの作成・破棄・ダウンロードは、それ自体が監査ログに残ります
- アーカイブの実体はバックアップにも含まれます（`ops/backup.sh` の対象範囲内）
...
| 監査ログの件数 | `SELECT count(*) FROM audit_log` | アーカイブが動いていれば1年ぶんで頭打ちになる |
| 画面の応答時間 | リバースプロキシのアクセスログ | 検索1秒（[実測](./capacity-report-2026-07.md)は337ms） |

### 応答時間の目安（名刺1万件・画像3万件での実測）

これを大きく上回る場合は、データ量の増加か、どこかで全件走査が起きている可能性があります。

| 画面 | 単独 | 5人同時＋取込2人 |
| --- | --- | --- |
| ホーム | 41 ms | — |
| 名刺一覧 | 161 ms | — |
| 検索 | 279 ms | 322 ms（最悪値） |
| CSV全件出力（1万件） | 2.85 秒 | — |

取込が走っていても検索は約7%しか遅くなりません（§5 の `OMP_THREAD_LIMIT=1` の効果）。
...
名刺10万件での実測では、**単独の応答時間より先に同時実行が限界に来ます**
（単独 最悪909ms に対し、5人同時では2.2秒）。年間3,000枚の想定では
10万件に達するのは30年以上先ですが、運用が変わって件数が跳ねた場合は
まずCPU増強を検討してください。索引では解決しません。詳細は
[実測報告 §7](./capacity-report-2026-07.md#7-10万件規模での測定と対策今後の課題1の検証)。
