...
**「名刺システム」フォルダ**が作られ、次の7つのショートカットが入ります。
あとはこのフォルダから番号順に実行してください。

| ショートカット | 内容 | tesseract |
| --- | --- | --- |
| 1 準備する（更新とセットアップ） | `git pull` → ショートカット作り直し → 実行環境の用意 | 不要 |
| 2 名刺を仕分ける | スキャンフォルダから名刺だけを取り出し、**結果のフォルダまで開く** | **必要** |
| 3 ラベル入力 | 実名刺に正解ラベルを付ける | 不要 |
| 4 精度を測る | **進み具合を出してから**正答率を測り、`real-rule.md` に出す | **必要** |
| アプリを起動 | 名刺管理システム本体を開く | **必要** |
| この1枚を調べる | 画像をドラッグ＆ドロップして、判定の根拠とOCRの読み取り文字を見る | **必要** |
| 動かないとき（診断） | 環境の確認結果を出す | 不要 |
...
**まず 1 を実行してください。** ここは tesseract が無くても動きます。
...
> 「1 準備する」を1回押せば、古いショートカットを消して作り直します
> （消えるのはショートカットだけで、名刺の画像やラベルには触れません）。

## 更新のしかた

**2回目以降は「1 準備する」を押すだけです。** `git pull`・ショートカットの
...
| 「名刺のフォルダがありません」 | 先に「2 名刺を仕分ける」を実行する |
