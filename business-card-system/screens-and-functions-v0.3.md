...
| 分類 | 項目 | 初期案 | 状態 |
| --- | --- | --- | --- |
| 性能 | 検索応答時間 | 1万件規模で**1秒以内**（実測を踏まえ短縮） | 実測 337ms（単独）／322ms（5人同時＋取込2人）。[実測報告](./capacity-report-2026-07.md) |
...
| 性能 | 同時利用者数 | 10名（うち同時取込2名） | 実測（検索5人＋取込2人）で最悪322ms。取込による悪化は約7%にとどまる。[実測報告](./capacity-report-2026-07.md#6-同時アクセス時の応答追加測定) |
| 可用性 | 稼働時間 | 平日8:00-20:00を主対象。SLA目標は未設定 | 要確定 |
| 可用性 | RPO / RTO | RPO 24時間、RTO **2営業時間**（4→2に短縮を提案） | 復元は実測1分（名刺1万件・画像1.1GB）。[実測報告](./capacity-report-2026-07.md)。決裁は[論点O](./open-issues-v0.3.md#その他の論点) |
| 性能 | CSV出力（1万件） | 10秒以内 | 実測 2.85秒（[実測報告](./capacity-report-2026-07.md)） |
