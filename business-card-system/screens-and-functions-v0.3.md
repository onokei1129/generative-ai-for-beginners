...
| SC-21 | 取込ジョブ一覧 | 取込待ち／OCR処理中／確認待ち／登録完了／エラーの状況表示 | 全員 | ◎ | §3 |
...
| FN-0316 | 取込状況表示 | 取込待ち／OCR処理中／確認待ち／登録完了／エラー | 全員 | ◎ | §3 |
...
| 性能 | OCR処理時間 | 1枚あたり10秒以内（確認待ちまで） | 要確定 |
| 性能 | 同時利用者数 | 10名（うち同時取込2名） | 要確定 |
| 可用性 | 稼働時間 | 平日8:00-20:00を主対象。SLA目標は未設定 | 要確定 |
| 可用性 | RPO / RTO | RPO 24時間、RTO 4営業時間 | [論点O](./open-issues-v0.3.md#その他の論点) |
| セキュリティ | 通信 | 全通信HTTPS | 確定 |
| セキュリティ | 保存データの暗号化 | ストレージ・DBの保存時暗号化を有効化 | 要確定 |
