...
tesseract があればOCRの下書きも入る（無ければ空欄のまま開く）。
...
**OCRが下書きを入れる**（実際のアプリと同じ流れ）。OCRが入れた欄は
黄色＋「未確認」で示され、触ると印が消える。次の名刺のOCRは裏で先に済ませるため、
2枚目以降は待たされない（実測 4.8秒 → 0.01秒）。
...
**黄色の欄は必ず確認すること。** OCRの誤りを見落としてそのまま正解にすると、
**測定値が実際より良く出る**。触らなかった欄は `_unverified` として記録され、
その正解ラベルで測ると `runner.py` が警告を出すようにしてある。
...
> このリポジトリでは Actions が有効になっており、PRにチェックが表示される。
> フォークを新しく作った場合は既定で無効なので、Settings → Actions で有効にすること。
...
| CI（自動テスト） | 実施済み | SQLite・PostgreSQL 16 の両方でテストとマイグレーションを毎回確認する（GitHub Actions） |
